...
# What you will need
#
# Packages: none. Use only stdlib (stdlib is what python gives you with no additional install).
#
...
def in_deadband(value, centre, width):
    if abs(value) <= abs(centre) + abs(width): return True
    else: return False

# 3. motor_command(error, deadband_width)
#    error is how far you are from the target. Positive means "too far
#    forward" in whatever units you are using; negative
#    means "too far backward".
...
def motor_command(error, deadband_width):
    if abs(error) <= abs(deadband_width): return "stop"
    elif error < 0: return 'backward'
    else: return 'forward'
