...
# it does not open a file. Function 3 has one new name: json.dumps.
#
...
# - JSON is a text format for lists and dictionaries. json.dumps turns a
#   Python list of dicts into that text.
...
def read_sensor_csv(filename: str) -> list[dict]:
    rows = []
    with open(filename) as f:
        header = None # <- this isn't explained in the summary
        for line in f:
            line = line.strip()
            if line == "":
            ...
            rows.append(row)
    return rows #<- these are all looping operations and  this is quite complex for a first task in this module. It should be changed
